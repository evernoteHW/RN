...
export const appIndexPageAction = () => {
	return dispatch => {
		dispatch({type: TYPES.CORE.PJB_APP_INDEX_PAGE_BEGGING})
	 	api
    .appIndexPage()
    .then(res => dispatch({'type': TYPES.CORE.PJB_APP_INDEX_PAGE_SUCCEED, res: res}))
    .catch (error => dispatch({type: TYPES.CORE.PJB_APP_INDEX_PAGE_FAILED}))
  }
}